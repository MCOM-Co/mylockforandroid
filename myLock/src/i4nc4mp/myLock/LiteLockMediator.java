...
public class LiteLockMediator extends Service {
...
/*Settings Flags*/	
	...
	public void onDestroy() {
		...
		pause();
//here, we might cause ourselves an issue if this stop is being called by the other mediator start
//TODO determine a failsafe
		if (patternsetting == 1) {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		//Need to send a stop intent for the full Mediator, to prevent any accidental conflicts
		Intent i = new Intent();
...
void activate() {
	...
	active = true;
	doToast(false);//pass notification of change
}
...
void pause() {
	...
	active = false;
	doToast(true);//pass notification of change
}

void doToast(boolean stopping) {
	//TODO lite mode should still start foreground but the notification is cleared by user
	...
	Toast.makeText(LiteLockMediator.this, update, Toast.LENGTH_LONG).show();	
